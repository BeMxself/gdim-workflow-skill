...
Kiro CLI 建议从 `~/.kiro/skills/**/SKILL.md`（用户级）加载 skills，并通过 agent profile 使用。

1) 将 skills 安装到用户目录（推荐且与自动化脚本一致）：
...
- `./run.sh --skip-tests`（跳过 `mvn test` 门禁，适合先改代码后统一跑测试）
- `./run.sh --stall-limit N`（连续 N 轮无 commit 判定 STALLED，默认 5）
...
**v1.6.1** - 自动提交覆盖 flow 目录所有 pending 文档，修复跨轮次文档更新导致的后续 clean-check 阻塞
