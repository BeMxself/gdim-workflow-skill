...
tmp_dir="$(mktemp -d)"
workflow_rel=".tmp-gap-final-decision-$$"
task_dir="${tmp_dir}/task"
output_file="${tmp_dir}/run.output"
workflow_abs="${PWD}/skills/${workflow_rel}/gap-final-flow"
...
mkdir -p "skills/${workflow_rel}/gap-final-flow"
...
cat >"${task_dir}/intents/01-gap-final-flow.md" <<'MD'
# intent
MD
...
GDIM_TEST_WORKFLOW_DIR="${workflow_abs}" \
  bash "${RUN_ROUND_SCRIPT}" \
    --flow-slug "gap-final-flow" \
    --max-rounds 1 \
    --workflow-dir "${workflow_rel}/gap-final-flow" \
    --intent-file "${task_dir}/intents/01-gap-final-flow.md" \
...
if [ "${exit_code}" -ne 0 ]; then
  echo "expected exit code 0 when gap decision is Generate Final Report, got ${exit_code}"
  cat "${output_file}"
  exit 1
fi

if ! grep -q "All gaps closed for gap-final-flow" "${output_file}"; then
  echo "expected round to close after final decision"
  cat "${output_file}"
  exit 1
fi

echo "PASS: final decision closes round"
