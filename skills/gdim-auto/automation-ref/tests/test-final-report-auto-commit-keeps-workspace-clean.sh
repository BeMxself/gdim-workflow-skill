...
workflow_rel=".ai-workflows/test-final-report-commit"
workflow_abs="${project_root}/${workflow_rel}/final-commit-flow"
...
cat >"${task_dir}/intents/01-final-commit-flow.md" <<'MD'
# intent
MD
...
(
  ...
  bash "automation/ai-coding/run-gdim-round.sh" \
    --flow-slug "final-commit-flow" \
    --max-rounds 1 \
    --workflow-dir "${workflow_rel}/final-commit-flow" \
    --intent-file "${task_dir}/intents/01-final-commit-flow.md" \
    --design-doc "docs/design/placeholder.md" \
    --modules "" \
    --allowed-paths "${workflow_rel}/final-commit-flow/" \
    --stage "B" \
...
if ! git -C "${project_root}" ls-files --error-unmatch "${workflow_rel}/final-commit-flow/99-final-report.md" >/dev/null 2>&1; then
  echo "expected 99-final-report.md to be committed"
...
if [ -n "$(git -C "${project_root}" status --porcelain)" ]; then
  echo "expected clean workspace after final-stage auto-commit"
  git -C "${project_root}" status --short
  cat "${output_file}"
  exit 1
fi

echo "PASS: final report is auto-committed and workspace stays clean"
