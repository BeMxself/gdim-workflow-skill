...
- `--skip-tests`：跳过 `mvn test` 门禁（也可用 `GDIM_SKIP_TESTS=1`）
- `--stall-limit N`：连续 N 轮无新 commit 则判定 `STALLED`（默认 `5`）
- `--no-auto-commit-gdim-docs`：关闭每轮 GDIM 文档自动提交（默认开启，可用 `GDIM_AUTO_COMMIT_GDIM_DOCS=0`）
...
| `./run.sh --skip-tests` | 跳过 `mvn test` 门禁（保留 compile/path/docs 检查） |
| `./run.sh --stall-limit 5` | 设置 STALLED 判定阈值（连续无 commit 轮数） |
| `./run.sh --no-auto-commit-gdim-docs` | 关闭每轮 GDIM 文档自动提交 |
...
- 默认进行编译/测试门禁（Maven 项目使用 `mvn compile/test -pl ... -am`）；如需“先写用例后统一跑测试”，可用 `--skip-tests`（或 `GDIM_SKIP_TESTS=1`）临时跳过测试门禁。
- Stage A 需要人工确认继续；Stage B 全自动；Stage C 用于收敛阶段。
- 默认按“每轮、每阶段独立会话”执行：同一 round 会依次触发 `scope → design → plan → execute → summary → gap` 6 次 runner 调用。
- 默认开启“每轮 GDIM 文档自动提交”（提交当前轮 scope/design/plan/summary/gap 等文档）；可通过 `--no-auto-commit-gdim-docs` 或 `GDIM_AUTO_COMMIT_GDIM_DOCS=0` 关闭。
- 阶段提示词按 `templates/stages/*.md.tpl` 分离维护，并显式注入该阶段必读输入文件（例如 design 注入 scope/intent/设计来源，plan 注入 design，execute 注入 plan，summary 注入 design+plan，gap 注入 intent+design+summary）。
- 每阶段执行前有输入文件硬校验；若必需输入缺失，会直接 `BLOCKED` 终止该 flow。
...
- 当执行器为 `kiro` 时，会先自动检查并确保存在 `gdim-kiro-opus` 与 `gdim-kiro-sonnet` 两个 agent（包含 gdim skill 资源）；在 `--ensure` 路径下会自动同步 gdim skills 到 `~/.kiro/skills/`。
